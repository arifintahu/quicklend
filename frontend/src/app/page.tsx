...
export default function Dashboard() {
  ...
  const handleAction = (amount: number, action: 'supply' | 'borrow') => {
    ...
    if (action === 'supply') {
      supply(selectedAsset.asset.asset.symbol, amount);
    } else {
      borrow(selectedAsset.asset.asset.symbol, amount);
    }
  ...
  const calculateProjectedHF = (amount: number, action: 'supply' | 'borrow', asset: MarketData) => {
      ...
      const existing = tempPositions.find(p => p.assetSymbol === asset.asset.symbol);
      
      if (action === 'supply') {
          if (existing) {
             existing.suppliedAmount += amount;
          } else {
             tempPositions.push({ assetSymbol: asset.asset.symbol, suppliedAmount: amount, borrowedAmount: 0, isCollateral: true });
          }
      } else {
           if (existing) {
             existing.borrowedAmount += amount;
           } else {
             tempPositions.push({ assetSymbol: asset.asset.symbol, suppliedAmount: 0, borrowedAmount: amount, isCollateral: false });
           }
      }
